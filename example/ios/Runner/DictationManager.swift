...
class DictationManager: NSObject, FlutterStreamHandler {
    ...
    private func handleStartListening(result: @escaping FlutterResult) {
        ...
        Task { @MainActor in
            ...
            do {
                ...
                print("🔴 LINE 210: Starting audio level streaming...")
                NSLog("🔴 LINE 210: Starting audio level streaming...")
                log("Starting audio level streaming...", level: .info)
                
                print("🔴 LINE 211: About to call startAudioLevelStreaming()")
                NSLog("🔴 LINE 211: About to call startAudioLevelStreaming()")
                log("About to call startAudioLevelStreaming()", level: .info)
                
                print("🔴 LINE 212: Current thread: \(Thread.isMainThread ? "MAIN" : "BACKGROUND")")
                NSLog("🔴 LINE 212: Current thread: \(Thread.isMainThread ? "MAIN" : "BACKGROUND")")
                log("Current thread: \(Thread.isMainThread ? "MAIN" : "BACKGROUND")", level: .info)
                
                print("🔴 LINE 213: About to invoke startAudioLevelStreaming() - this log should appear")
                NSLog("🔴 LINE 213: About to invoke startAudioLevelStreaming() - this log should appear")
                log("About to invoke startAudioLevelStreaming() - this log should appear", level: .info)
                
                print("🔴 LINE 214: Entering do block")
                NSLog("🔴 LINE 214: Entering do block")
                do {
                    print("🔴 LINE 215: About to call startAudioLevelStreaming() function")
                    NSLog("🔴 LINE 215: About to call startAudioLevelStreaming() function")
                    startAudioLevelStreaming()
                    print("🔴 LINE 216: startAudioLevelStreaming() call completed successfully")
                    NSLog("🔴 LINE 216: startAudioLevelStreaming() call completed successfully")
                    log("startAudioLevelStreaming() call completed successfully", level: .info)
                } catch {
                    print("🔴 LINE 218: ERROR: Exception caught in startAudioLevelStreaming(): \(error)")
                    NSLog("🔴 LINE 218: ERROR: Exception caught in startAudioLevelStreaming(): \(error)")
                    log("ERROR: Exception caught in startAudioLevelStreaming(): \(error)", level: .error)
                    log("Error type: \(type(of: error))", level: .error)
                }
                print("🔴 LINE 221: After startAudioLevelStreaming() call - this log should also appear")
                NSLog("🔴 LINE 221: After startAudioLevelStreaming() call - this log should also appear")
    ...
    private func startAudioLevelStreaming() {
        ...
        print("🔴🔴🔴 CRITICAL LINE 409: startAudioLevelStreaming() FUNCTION ENTRY - THIS MUST APPEAR 🔴🔴🔴")
        NSLog("🔴🔴🔴 CRITICAL LINE 409: startAudioLevelStreaming() FUNCTION ENTRY - THIS MUST APPEAR 🔴🔴🔴")
